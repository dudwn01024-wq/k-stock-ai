...
function server({productionVolume=false,missingSupply=false,missingVolume=false,missingTrend=false,missingNews=false,priceRows=null,kisLoader=null,serverSource=source}={}) {
  ...
  const ctx=vm.createContext({process:{env:{GEMINI_API_KEY:'MOCK_ONLY'}},console,AbortController,setTimeout,clearTimeout,
    require:name=>{
      ...
      if(name==='./services/kisVolumeData' && productionVolume)return {assessVolume:require('../services/kisVolumeData').createRealtimeVolumeAdapter({getSnapshot:()=>null},()=>new Date('2026-09-18T10:30:00+09:00')).assess};
