...
const market=()=>({volume:150,averageVolume20:100,foreignerNet:1,institutionNet:1,newsAssessment:{hasCautionSignal:false}});
...
test('zero volume is a measurement; null, whitespace and arrays are not zero',()=>{
  for(const v of [null,undefined,'',' ',[],{},false,NaN]) {
    const m=engine.evaluateMarketContext({...market(),volume:v});assert.equal(m.volumeRatio,null);assert.equal(m.conditions.volume.status,'UNAVAILABLE');
  }
  assert.equal(engine.evaluateMarketContext({...market(),volume:0}).volumeRatio,0);
});
test('advanced volume thresholds and neutral band remain unchanged',()=>{
  for(const [volume,status] of [[150,'FAVORABLE'],[149,'NEUTRAL'],[71,'NEUTRAL'],[70,'CAUTION']])
    assert.equal(engine.evaluateMarketContext({...market(),volume}).conditions.volume.status,status);
});
...
function kisAdapter(raw, pageSize=Infinity) {
  ...
  const context=vm.createContext({module:{exports:{}},URL,console,AbortController,setTimeout,clearTimeout,
    ...
    require:name=>{assert.equal(name,'./dataFreshness');return require('../services/dataFreshness');},
    fetch:async url=>{
      const u=new URL(url);
